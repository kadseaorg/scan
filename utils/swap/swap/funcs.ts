...
import {
	...
	SwapChainWithExactOutputParams,
	SwapSingleWithExactInputParams,
	SwapSingleWithExactOutputParams,
} from './types'

export const getSwapContract = (address: string, web3: Web3): any => {
...
export const getSwapSingleWithExactInputCall = (
	...
	if (strictERC20Token == undefined) {
		...
		inputIsChainCoin =
			!strictERC20Token && isGasOrWrappedGasToken(params.inputToken, chain.id)
		outputIsChainCoin =
			!strictERC20Token && isGasOrWrappedGasToken(params.outputToken, chain.id)
	}
	...
	const innerRecipientAddress = outputIsChainCoin
	...
	return {
		swapCalling: swapContract.methods.multicall(multicall),
		options: buildSendingParams(chain, options, gasPrice),
	}
}
...
export const getSwapChainWithExactInputCall = (
	...
	if (strictERC20Token == undefined) {
		...
		inputIsChainCoin =
			!strictERC20Token && isGasOrWrappedGasToken(inputToken, chain.id)
		outputIsChainCoin =
			!strictERC20Token && isGasOrWrappedGasToken(outputToken, chain.id)
	}
	if (inputIsChainCoin) {
		// options.value = params.inputAmount;
		options.value = `0x${Number(params.inputAmount).toString()}`
	}
	const finalRecipientAddress = params.recipient ?? account
	const innerRecipientAddress = outputIsChainCoin
		? '0x0000000000000000000000000000000000000000'
