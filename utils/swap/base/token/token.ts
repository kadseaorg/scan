...
export const isGasOrWrappedGasToken = (
	token: TokenInfoFormatted,
	chainId: ChainId,
): boolean => {
	if (!token || !chainId) {
...
export const isWrappedGasToken = (
	token: TokenInfoFormatted,
	chainId: ChainId,
): boolean => {
	if (!token || !chainId) {
...
export const getWrappedGasTokenIfExists = (
	chainId: ChainId,
): TokenInfoFormatted => {
	const chain = getChain(chainId)
	if (
		!!chain?.wrappedToken?.address &&
		!!chain?.wrappedToken?.symbol &&
		!!chain?.wrappedToken?.decimal
	) {
		return {
			chainId,
